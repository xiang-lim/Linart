...
def calculate_y_point_of_contact_with_circle(radius, coord_of_pixel, radian):
    dist_of_pixel_from_center = cal_euclidean_distance((radius, radius), coord_of_pixel)
    radian_of_pixel_with_respect_to_horizon = math.atan2(coord_of_pixel[0], coord_of_pixel[1])
    ...
    if math.tan(radian) == math.pi or math.tan(radian) == -math.pi:
        ...
        y1 = radius * math.cos(radian_of_pixel_with_respect_to_horizon - radian_of_half_sector) + \
             radius if coord_of_pixel[0] > radius else (-radius)
...
def determine_closest_coord(width, circle_point_df, coord1, radius):
    ...
    coord1_x = big_x if abs(coord1[1] - small_x) > abs(coord1[1] - big_x) else small_x
    df_y = circle_point_df[circle_point_df['x'] == coord1_x]

    if len(df_y) > 1:
        coords_y = df_y.ye.to_list()
        coord_y = coords_y[1] if abs(coords_y[0] - coord1[0]) > abs(coords_y[1] - coord1[0]) else coords_y[0]
    else:
        coord_y = df_y.ye
...
def determine_coordinates_of_line_intercept_with_circle_point(diameter, m_o_matrix, circle_point_df, height, width):
    radius = diameter / 2
    list_of_coord = []
    index = 0
    for row in m_o_matrix:
        ...
        for pixel in row:
            if pixel != 0:
                radian = pixel[1]
                coord1, coord2 = calculate_y_point_of_contact_with_circle(radius, (
                    pixel[2] + radius - height / 2, pixel[3] + radius - width / 2), radian)
                coord1 = determine_closest_coord(diameter, circle_point_df, coord1, radius)
                coord2 = determine_closest_coord(diameter, circle_point_df, coord2, radius)
...
def visualize_string_art_v2(magnitude_orientation_matrix):
    ...
    diameter_of_image = determine_diameter_of_image(
        height, width
    )
    circle_coord_df = determine_360_points(diameter_of_image)
    print(circle_coord_df)
    determine_coordinates_of_line_intercept_with_circle_point(diameter_of_image, magnitude_orientation_matrix,
                                                              circle_coord_df, height, width)
    # plt.figure(figsize=(10,10))
